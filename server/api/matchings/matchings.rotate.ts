...
export default defineEventHandler(async (event) => {
  ...
  const ring = await prisma.group.findMany({
  ...
  const N = ids.length;
  if (N === 0)
    throw createError({ statusCode: 400, statusMessage: "No groups in ring" });

  const jobs = jobIds?.length
    ? await prisma.job.findMany({
        where: { id: { in: jobIds } },
        select: { id: true, rotationPointer: true },
      })
    : await prisma.job.findMany({
        select: { id: true, rotationPointer: true },
      });
  ...
  await prisma.$transaction([
    ...updates.map((u) =>
      prisma.job.update({
        where: { id: u.jobId },
        data: { rotationPointer: u.pointer },
      }),
    ),
    ...updates.map((u) =>
      prisma.groupJob.upsert({
        where: { jobId: u.jobId },
        create: { jobId: u.jobId, groupId: u.groupId },
