...
describe("POST /api/group", () => {
  ...
  test("Group can be deleted successfully", async () => {
    await (await agent).delete(`/api/group/delete`).send({ id }).expect(200);

    // Verify that the group has been deleted
    await (await agent).get(`/api/group/get`).query({ id }).expect(404);
  });
  ...
  test("Created group appears in the list of groups", async () => {
    ...
    const res = await (await agent).get("/api/groups").expect(200);
